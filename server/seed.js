    ...
    for (const sub of subcategories) {
      ...
      if (!found) {
        try {
          ...
          if (err.code === 11000) {
            console.log(`Duplicate subcategory skipped: ${sub.name} (${sub.category})`);
          } else {
            throw err;
          }
        }
      } else {
        console.log(`Subcategory already exists: ${sub.name} (${sub.category})`);
        createdSubcategories.push(found);
      }
    }
    ...
    const products = [
      {
        ...
        price: 120,
        stockQuantity: 10,
        description: "Класическа елегантна люстра.",
        images: [],
        supplier: "Доставчик 1",
        productCode: "LK-001"
      },
      {
        name: "Настолна лампа Модерн",
        category: categoryMap["Настолни лампи"],
        subcategory: subcategoryMap["Модерни"],
        price: 45,
        stockQuantity: 25,
        description: "Модерна настолна лампа за бюро.",
        images: [],
        supplier: "Доставчик 2",
        productCode: "NL-002"
      }
      // Може да добавите още продукти по желание
    ];
