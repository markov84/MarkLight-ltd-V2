...
export default function Profile() {
  ...
  useEffect(() => {
    ...
    const fetchProfile = async () => {
      try {
        const res = await axios.get(`${API}/api/auth/profile`, { withCredentials: true });
    ...
    const fetchFavorites = async () => {
      try {
        setFavLoading(true);
        const res = await axios.get(`${API}/api/auth/favorites`, { withCredentials: true });
          ...
          {profile ? (
            <div className="space-y-4">
              ...
              <div className="flex gap-4 mt-6">
                <button onClick={handleLogout} className="bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-lg">Изход</button>
